...
#let ieee(
  ...
  // We're keeping the parameter name as 'index-terms' for consistency with IEEE template structure,
  // but the displayed text will be 'Keywords'.
  index-terms: (),
  ...
  // Display abstract and index terms (now labeled as Keywords).
  if abstract != none {
    ...
    if index-terms != () {
      // Changed 'keywords' back to 'index-terms' here for consistency with the parameter
      parbreak()
      // MODIFICATION HERE: Changed "Index Terms" to "Keywords"
      [_Keywords_---#h(weak: true, 0pt)#index-terms.join[, ]]
    }
    v(2pt)
  }
